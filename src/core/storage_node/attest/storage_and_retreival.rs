...
mod tests {
    ...
    fn test_initialization() {
        let storage_node = Arc::new(
            StorageNode::new([0u8; 32], 1000, Default::default(), HashSet::new()).unwrap(),
        );

        let propagation: BatteryPropagation<()> = BatteryPropagation::new(
            storage_node,
        ...
        assert_eq!(propagation.get_battery_level(), 0);
        assert_eq!(propagation.get_battery_boost(), 0);
        assert_eq!(propagation.get_synchronization_boost(), 0);
    }
}
