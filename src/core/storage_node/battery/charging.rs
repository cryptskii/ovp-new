...
impl BatteryChargingSystem {
    pub fn new(config: StorageNodeConfig, stake_amount: u64) -> Self {
        ...
        Self {
            battery_level: AtomicU64::new(config.max_battery),
            config,
    ...
    pub async fn recharge(&self, synchronized_nodes: u64) -> Result<(), SystemErrorType> {
        ...
        if current_level >= self.config.max_battery {
            return Ok(());
        }

        let charge_rate = self.config.charge_rate * synchronized_nodes;
        let new_level = current_level.saturating_add(charge_rate);
        let capped_level = new_level.min(self.config.max_battery);
    ...
    pub fn get_charge_percentage(&self) -> f64 {
        ...
        (current_level as f64 / self.config.max_battery as f64) * 100.0
    }
            ...
            let promise = js_sys::Promise::new(&mut |resolve, _| {
                window()
                    ...
                    .set_timeout_with_callback_and_timeout_and_arguments_0(
                        &resolve,
                        self.config.charge_wait_ms as i32,
                    )
                    .unwrap();
            });
